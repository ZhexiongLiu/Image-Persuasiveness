...
if __name__ == '__main__':
    ...
    if args.exp_mode == 1:
        ...
        if args.img_model == 0:
            init_model = ImageModelResNet50(out_dim=6)
        elif args.img_model == 1:
            init_model = ImageModelResNet101(out_dim=6)
        else:
            init_model = ImageModelVGG16(out_dim=6)
        criterion = nn.CrossEntropyLoss()
    elif args.exp_mode == 3:
        # multi-label classification
        if args.img_model == 0:
            init_model = ImageModelResNet50(out_dim=3)
        elif args.img_model == 1:
            init_model = ImageModelResNet101(out_dim=3)
        else:
            init_model = ImageModelVGG16(out_dim=3)
        criterion = nn.BCEWithLogitsLoss()
    else:
        # binary classification
        if args.img_model == 0:
            init_model = ImageModelResNet50(out_dim=1)
        elif args.img_model == 1:
            init_model = ImageModelResNet101(out_dim=1)
        else:
            init_model = ImageModelVGG16(out_dim=1)
        criterion = nn.BCEWithLogitsLoss()
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(df)):
        ...
        train_dataset = ImageDataset(args, annotation=train_annotation, root_dir=os.path.join(args.data_dir, 'images'), transform=train_transform)
        val_dataset = ImageDataset(args, annotation=val_annotation, root_dir=os.path.join(args.data_dir, 'images'), transform=val_transform)
