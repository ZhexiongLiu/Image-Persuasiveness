...
def report_res(subdir, exp_name, is_majority=False):
    f1_list = []
    precision_list = []
    recall_list = []
    acc_list = []
...
def get_res_table():
    ...
    for subdir in glob.glob(os.path.join("./experiments","*/")):
        ...
        exp_res = report_res(subdir, exp_name)
        exp_majority_res = report_res(subdir, exp_name, is_majority=True)

        exp_res = exp_res.split(" ")
        exp_majority_res = exp_majority_res.split(" ")
        res_str = f"{exp_name} & {exp_res[0]} & {exp_res[1]} & {exp_res[2]} & {exp_res[3]} & {exp_res[4]}"
        res_majority_str = f"{exp_name} & {exp_majority_res[0]} & {exp_majority_res[1]} & {exp_majority_res[2]} & {exp_majority_res[3]} & {exp_majority_res[4]}"
